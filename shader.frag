...
void main() {
	...
	value = 0.25 + 0.9*value;
    
    // if px position is near mousepos
    // increase value
    
    float dist = pow(distance(mousePos, p), 1.0);
    dist = 1.0 - quadraticInOut(dist+0.3);
   ...
   value += dist*0.2;
    ...
    if (value <= 0.5) {
        color = mix(colorA, colorB, value * 2.0);
    
    } else {
        color = mix(colorB, colorC, (value -0.5) * 2.0 );
    
    }
       
   
   // added a bit of grain on top
   color += random(p, u_time * 100.0) * 0.08;
